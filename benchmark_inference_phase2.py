...
if __name__ == "__main__":
    ...
    for batch_size in [1, 8, 16]:
        for kv_caching in [False, True]:
            for i in range(10):
                record = benchmark_inference(batch_size=batch_size, input_len=input_len, output_len=output_len, kv_caching=kv_caching)
    ...
    import pickle
    import os 
    # add timestamp to the filename in a folder named benchmark_results
    if not os.path.exists("benchmark_phase2_results"):
        os.makedirs("benchmark_phase2_results")
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    filename = f"benchmark_phase2_results_{timestamp}.pkl"
